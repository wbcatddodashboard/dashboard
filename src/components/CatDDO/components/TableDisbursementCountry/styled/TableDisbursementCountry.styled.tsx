...
export function TableDisbursementCountryContainer({ children }: ChildrenProps) {
  return <div className="w-full">{children}</div>;
}

export function TableDisbursementCountryWrapper_Container({
  children,
}: ChildrenProps) {
  return <div className="w-full">{children}</div>;
}
...
export interface TableDisbursementCountryWrapperProps<T = unknown> {
  data: T[];
...
export const TableDisbursementCountryWrapper = <T,>(
  props: TableDisbursementCountryWrapperProps<T>
) => {
  const {
    maxBodyHeight = 360,
    scroll,
    ...rest
  } = props as TableDisbursementCountryWrapperProps<T>;
