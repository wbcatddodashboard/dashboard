...
export function ChartContainer({ children }: ChildrenProps) {
  return (
    <div className="box-border content-stretch flex flex-col gap-4 items-start justify-start p-0 relative shrink-0 w-[307px]">
      {children}
    </div>
  );
}
...
export function ChartWrapper({ children }: ChildrenProps) {
  return (
    <div className="bg-[#ffffff] h-[389px] overflow-clip relative shrink-0 w-[285px]">
      {children}
    </div>
  );
}
